...
func main() {
	...
	for i := 0; i < len(list); i++ {
		// fmt.Sprintln just returns the string, which we were ignoring here.  It doesn't send it to stdout/log.
		// fmt.Print* will send the string to stdout so we can see it.
		// fmt.Sprintln(list[i])
		fmt.Println(list[i])
	}
}
